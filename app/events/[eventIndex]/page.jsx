    ...
    <main>
        <section className='py-12 px-10 max-sm:p-8 bg-primary-maroon'>
          <div className='relative'>
            <h1 className='font-caudex text-4xl max-md:text-2xl max-sm:text-xl font-bold text-center text-beige'>{eventpics[eventIndex].title}</h1>
            <Link href="/events">
        ...
        <section className='flex justify-center bg-beige'>
          <div className='w-1/2 max-md:w-full'>
            <Carousel autoSlide={true} autoSlideInterval={4000}>
              {eventpics[eventIndex].carouselpics.map((item,i) => (
        ...
        <section>
          <div className='px-48 max-md:px-12 mt-16 max-container'>
              {eventpics[eventIndex].content.map((item,i) => (
                <div key={i}>
              ...
              {eventpics[eventIndex].ytlink && (
                <div>
                  <Link href={eventpics[eventIndex].ytlink} className='font-palanquin text-lg text-teal-600 max-sm:text-base mt-6 underline underline-offset-4'>
                    {eventpics[eventIndex].ytcontent}
                  </Link>
                </div>
              )}
              <hr className="my-6 w-full h-px bg-gray-300 border-0 rounded"/>
                <p className='mb-12 font-palanquin text-left text-primary-maroon text-xl max-sm:text-base max-sm:leading-7 leading-8'>{eventpics[eventIndex].date}</p>
