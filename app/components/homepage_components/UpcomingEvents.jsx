...
const UpcomingEvents = ({ props }) => {
  ...
  return (
    <section>
      <div>
        <h1 className="mb-10 text-primary-maroon font-caudex text-center text-4xl max-md:text-3xl max-md:leading-8 font-bold">
          {" "}
          Upcoming Events{" "}
        </h1>
        <div>
            ...
            <div className="flex font-palanquin flex-row justify-center gap-8 max-sm:flex-col">
              {recent.map((item, i) => (
                <div className="rounded shadow-lg h-[500px] w-1/3 max-lg:w-1/2 max-md:w-full bg-white">
                  <img
                    className="shadow-lg w-full h-3/5 max-lg:h-1/2 max-sm:h-3/5 object-cover object-top"
                    src={item.imgURL[0]}
                  ...
                  <div className="h-2/5 max-lg:h-1/2 max-sm:h-2/5">
                    <div className="px-6 pt-4 leading-8">
                      ...
                      <div className="mt-3 leading-7">
                        Time: {item.time} <br /> Date: {item.date}{" "}
                      </div>
                    </div>
                    <div className="p-5">
                      <Link href={`/events/upcomingevents/${i}`}>
                        <Button label="Click to Join" type="" />
                      </Link>
                    </div>
                  </div>
                </div>
